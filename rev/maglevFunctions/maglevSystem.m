...
classdef maglevSystem < handle
   ...
   methods
        function obj = maglevSystem(x0, params, approximationType, eccentricity)
            mu0 = 4*pi*1e-7;
            obj.approximationType = approximationType;
            bsqr = 1 / (1 - eccentricity^2); % multiplier for the length of the y axis of the ellipse
            
            %% Solenoids
            Xs = params.solenoids.R*cos(linspace(0,2*pi,params.solenoids.N+1));
            Ys = params.solenoids.R*bsqr*sin(linspace(0,2*pi,params.solenoids.N+1));
            Zs = zeros(size(Xs));
            ...
            Ypm = params.magnets.R*bsqr*sin(linspace(0,2*pi,params.magnets.N+1)+params.magnets.offset);
            Zpm = zeros(size(Xpm));
